...
from ilvrefwi.regularization import *
from ilvrefwi.utils import *
from scipy.optimize import linear_sum_assignment
from scipy.signal import butter
from scipy.ndimage import gaussian_filter
# from ilvrefwi.cnn import *
...
def fwi_loop(
    ...
    learning_rate=2, optim='adam', save_dir=None, 
    maxs=[5000,2887,2607], mins=[3000,1732,2294], num_batches=2,
    synthetic_mask=None, elastic=True, gradient=False, 
    grad_norm=False, grad_smooth=None, use_scheduler=False, 
    inv_source=False, source_illum=None, grad_clip=1.,
    regularization=None, regularization_weight=0.0, gradient_mask=None,
    cnn=None, volve=False):
    """
    ...
    for epoch in range(num_epochs):
        ...
        for it in range(0, num_shots, num_batches):
            ...
            if elastic:
                ...
                batch_rcv_amps_pred = deepwave.elastic(
                    *deepwave.common.vpvsrho_to_lambmubuoyancy(vp_smooth, vs_smooth, rho_smooth),
                    [dz,dx], dt, source_amplitudes_y=batch_src_amps,
                    source_locations_y=batch_source_locations,
                    receiver_locations_p=batch_receiver_locations,
                    accuracy=4,
                    pml_freq=freq,
                    pml_width=[50, 50, 50, 50]
                )[-3]#[:,:,::2]
            ...
            if filter is not None:
                ...
                for cutoff_freq in [filter]:
                    sos = butter(freq, cutoff_freq, fs=1/dt, btype='highpass', output='sos')
                    sos = [torch.tensor(sosi).to(batch_rcv_amps_true.dtype).to(device) for sosi in sos]
                    def filt(x):
                        return biquad(biquad(biquad(x, *sos[0]), *sos[1]), *sos[2])
                    
                batch_rcv_amps_pred = filt(batch_rcv_amps_pred)
                batch_rcv_amps_true = filt(batch_rcv_amps_true)
            ...
            if loss_type=='l2':
                # print(batch_rcv_amps_pred.shape, mask[0].shape)
                # print(batch_rcv_amps_pred.shape, mask[1].shape)
        ...
        # Gradient masking for water layer
        if gradient_mask is not None:
            vp_smooth.grad *= gradient_mask
            vs_smooth.grad *= gradient_mask
            rho_smooth.grad *= gradient_mask
            
        # Smooth gradient
        if grad_smooth is not None:
            vp_smooth.grad = torch.tensor(gaussian_filter(vp_smooth.grad.cpu().numpy(), [2, grad_smooth])).to(device)
            vs_smooth.grad = torch.tensor(gaussian_filter(vs_smooth.grad.cpu().numpy(), [2, grad_smooth])).to(device)
            rho_smooth.grad = torch.tensor(gaussian_filter(rho_smooth.grad.cpu().numpy(), [2, grad_smooth])).to(device)
        ...
        if cnn is not None:
            vp_n = normalize_vp(vp_smooth.data, vmin=mins[0], vmax=maxs[0])
            vs_n = normalize_vs(vs_smooth.data, vmin=mins[1], vmax=maxs[1])
            rho_n = normalize_rho(rho_smooth.data, vmin=mins[2], vmax=maxs[2])
            recon = cnn_reconstruction(
                resize(vp_n.detach().cpu().numpy(), (256,256)).reshape(1,256,256), 
                resize(vs_n.detach().cpu().numpy(), (256,256)).reshape(1,256,256), 
                resize(rho_n.detach().cpu().numpy(), (256,256)).reshape(1,256,256), 
                cnn
            )
            recon = torch.from_numpy(resize(recon, (3, vp_smooth.data.shape[0], vp_smooth.data.shape[1]))).float().to(device)
            vp_smooth.data = denormalize_vp(recon[0], vmin=mins[0], vmax=maxs[0])
            vs_smooth.data = denormalize_vs(recon[1], vmin=mins[1], vmax=maxs[1])
            rho_smooth.data = denormalize_rho(recon[2], vmin=mins[2], vmax=maxs[2])
