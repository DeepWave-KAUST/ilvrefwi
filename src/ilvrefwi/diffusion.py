...
from ilvrefwi.fwi import fwi_loop

from ilvrefwi.resizer import Resizer
...
class DenoisingDiffusionProbabilisticModel:
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, shape, batches=False, resizers_img=None, 
                      resizers_xt=None, range_t=0, ref_img=0): # p_sample_loop_progressive in gaussian_diffusion.py
        """
        ...
        for i in tqdm(reversed(range(0, self.timesteps)), total=self.timesteps):
            ...
            # ILVR
            if resizers_img is not None:
                t = torch.tensor([i] * shape[0], device=device)
                down, up = resizers_img
                
                if resizers_xt is not None:
                    down, up = resizers_xt
                else:
                    down, up = resizers_img
                ...
                if i > range_t:
                    img = (
                        img 
                        - up(down(img)) 
                        + up(down(self.q_sample(ref_img, t, torch.randn(*shape, device=device)))) #th.randn(*shape, device=device)
                    )
    ...
    def sample(self, model, image_size, batch_size=8, channels=3, batches=False, 
               resizers_img=None, resizers_xt=None, range_t=0, ref_img=0):
        """
        ...
        return self.p_sample_loop(
            ...
            batches=batches, 
            resizers_img=resizers_img, resizers_xt=resizers_xt, 
            range_t=range_t, ref_img=ref_img
        )
    ...
    def fwi_sample(self, model, shape, init_time_step, 
                   ...
                   learning_rate=20, optim='adam', diffusion='n', 
                   ...
                   regularization=None, regularization_weight=0.0, cnn=None,
                   ilvr_sampling=False, num_particles=1, down_n=[0], ilvr_weight=0.05, 
                   gradient_mask=None, volve=False):
        """
        ...
        for i in tqdm(reversed(range(0, self.timesteps-init_time_step)),
            ...
            if (diffusion=='y') or (diffusion==True):
                img = torch.cat((vp_img, vs_img, rho_img), 1)
                
                # Duplicate to generate several samples at once
                img = img.repeat(num_particles,1,1,1)
                
                img = torch.from_numpy(resize(img.detach().cpu().numpy(), (img.shape[0],img.shape[1],image_size,image_size))).to(device)
                ...
                # ILVR half-way
                if (ilvr_sampling=='y'):# & (i<(self.timesteps-init_time_step)//2): 
                    
                    # print('img_mean before\n')
                    # print(img_mean.shape)
                    # print(img_mean.min())
                    # print(img_mean.max())
                    
                    print(f'EFWI with Diffusion Priors and ILVR Sampling.')
                    # Subsampling factor
                    down_ni = down_n[i]
                    ...
                    # Define Resizer
                    down = Resizer((num_particles, 3, image_size, image_size), 1 / down_ni).to(device)
                    up = Resizer((num_particles, 3, int(image_size / down_ni), int(image_size / down_ni)), down_ni).to(device)

                    new_shape = (num_particles, 3, image_size, image_size)
                    
                    # print('img mean\n')
                    # plt.figure()
                    # plt.imshow(img_mean[0,0].detach().cpu().numpy())
                    # plt.colorbar()
                    # plt.show()
                    
                    img_mean = (
                        img_mean 
                        - ilvr_weight*up(down(img_mean)) 
                        + ilvr_weight*up(down(self.q_sample(img, torch.tensor([i] * num_particles, device=device), torch.randn(*new_shape, device=device))))
                    )
                    
                    # print('img N\n')
                    # plt.figure()
                    # plt.imshow(up(down(self.q_sample(img, torch.tensor([i] * num_particles, device=device), torch.randn(*new_shape, device=device))))[0,0].detach().cpu().numpy())
                    # plt.colorbar()
                    # plt.show()
                    
                    # print('img_mean N\n')
                    # plt.figure()
                    # plt.imshow(up(down(img_mean))[0,0].detach().cpu().numpy())
                    # plt.colorbar()
                    # plt.show()
                    
                    # print('img_mean after\n')
                    # print(img_mean.shape)
                    # print(img_mean.min())
                    # print(img_mean.max())
                ...
                if num_particles>1:
                    np.save(save_dir+'samples_ex2_it_'+str(i)+'.npy', img_mean.detach().cpu().numpy())
                    img_mean = img_mean[0].reshape(1,img.shape[1],vp_img.shape[2],vp_img.shape[3])
                ...
                if diffusion_only==False:
                    if i != 0:
                        vp_img, vs_img, rho_img, loss_fwi, data_loss = fwi_loop(
                            ...
                            freq, num_epochs, data_weight, save_dir=save_dir, maxs=maxs, mins=mins, filter=filter,
                            ...
                            learning_rate=learning_rate, gradient_mask=gradient_mask, volve=volve
                    )
            elif cnn is not None:
                print(f'EFWI with CNN Priors.')
                img = torch.cat((vp_img, vs_img, rho_img), 1)
                vp_img, vs_img, rho_img, loss_fwi, data_loss = fwi_loop(
                    ...
                    grad_smooth=grad_smooth,inv_source=inv_source,source_illum=source_illum,
                    grad_clip=grad_clip, regularization=regularization, regularization_weight=regularization_weight,
                    cnn=cnn, learning_rate=learning_rate, gradient_mask=gradient_mask, volve=volve
                )
            else:
