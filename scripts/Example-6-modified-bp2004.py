...
from ilvrefwi.utils_salt import *
...
if __name__ == "__main__":
    ...
    source_spacing = 199 #30.0
    receiver_spacing = 20.0
    device = torch.device('cuda')
    
    vp_true = torch.from_numpy(np.load('../data/vp_mod_bp04.npy').reshape(225,400)).float().to(device)
    vs_true = torch.from_numpy(np.load('../data/vs_mod_bp04.npy').reshape(225,400)).float().to(device)
    rho_true = torch.from_numpy(np.load('../data/rho_mod_bp04.npy').reshape(225,400)).float().to(device)
    
    if args.shift_trues=='y':
    ...
    vp_init = torch.from_numpy(gaussian_filter(vp_true.detach().cpu().numpy(), sigma=[args.sigma,args.sigma])).to(device)
    vs_init = torch.from_numpy(gaussian_filter(vs_true.detach().cpu().numpy(), sigma=[args.sigma,args.sigma])).to(device)
    rho_init = torch.from_numpy(gaussian_filter(rho_true.detach().cpu().numpy(), sigma=[args.sigma,args.sigma])).to(device)
    ...
    receiver_amplitudes_true = out + args.noise_level*torch.randn_like(out)
    ...
    model.load_state_dict(torch.load('../saves/diffusion/reseemsalt-25616-new-model-large-ddpm_15.pt'))
    model.to(device)
    timesteps=1000
    ...
    fwi_dict = {
        ...
        'receiver_amplitudes_true': receiver_amplitudes_true+1e-8*torch.randn_like(receiver_amplitudes_true)
    }
    ...
    vp_diff,vs_diff,rho_diff,loss_fwi_diffusionFWI = ddpm.fwi_sample(
        ...
        ilvr_sampling=args.ilvr_sampling, num_particles=args.num_particles, regularization=args.regularization, regularization_weight=args.regularization_weight,
        down_n=list(np.repeat(Ns, int((timesteps-start_timesteps)/len(Ns)))), ilvr_weight=args.ilvr_weight, gradient_mask=mask
        # filter=3
    )
