...
if __name__ == "__main__":
    ...
    vp_true = torch.from_numpy(np.load('../data/vp_mod_over.npy').reshape(225,400)).float().to(device)
    vs_true = vp_to_vs(vp_true)
    rho_true = vp_to_rho(vp_true)*1e3
    
    vp_true = torch.from_numpy(gaussian_filter(vp_true.detach().cpu().numpy(), sigma=[3,3])).to(device)
    vs_true = torch.from_numpy(gaussian_filter(vs_true.detach().cpu().numpy(), sigma=[3,3])).to(device)
    rho_true = torch.from_numpy(gaussian_filter(rho_true.detach().cpu().numpy(), sigma=[3,3])).to(device)
    ...
    model.load_state_dict(torch.load('../saves/diffusion/model_45.pt'))
